...
//        UpdateComponent(_componentList[i], _spriteList[no], _color);
        UpdateComponent(_componentList[i], _spriteList[no], _textSize);
        }
    }

    //各Componentを更新
//    protected abstract void UpdateComponent(T component, Sprite sprite, Color color);
    protected abstract void UpdateComponent(T component, Sprite sprite, float textSize);
    ...
    // 文字のサイズを変更する
    public void ChangeSize(float textSize)
    {
        _textSize = textSize;
        UpdateSprites();
    }
